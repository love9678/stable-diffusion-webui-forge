...
case "$gpu_info" in
    ...
    *"Navi 3"*) [[ -z "${TORCH_COMMAND}" ]] && \
         export TORCH_COMMAND="pip install torch torchvision --index-urlhttps://pypi.tuna.tsinghua.edu.cn/whl/nightly/rocm5.7"
    ;;
...
then
    ...
    then
	      export TORCH_COMMAND="pip install torch torchvision torchaudio --index-url https://pypi.tuna.tsinghua.edu.cn/whl/rocm5.7"
    elif npu-smi info 2>/dev/null
    then
        export TORCH_COMMAND="pip install torch==2.1.0 torchvision torchaudio --index-url https://pypi.tuna.tsinghua.edu.cn/whl/cpu; pip install torch_npu==2.1.0"
    fi
fi
